...
use reec_core::{
    ...
    types::{AccountInfo, Block, BlockHeader, BlockNumber, GenericTransaction, Receipt, Transaction, TxKind, Withdrawal, GWEI_TO_WEI, INITIAL_BASE_FEE},
};
use reec_storage::{error::StoreError, AccountUpdate, Store};
use lazy_static::lazy_static;
use revm::{
    Database, DatabaseCommit, Evm, db::{states::bundle_state::BundleRetention, AccountStatus}, 
...
use revm_primitives::{ruint::Uint, AccessList as RevmAccessList, AccessListItem as RevmAccessListItem, TxKind as RevmTxKind, };
...
/// Executes all transactions in a block and performs the state transition on the database and stores the block in the DB
pub fn execute_block(block: &Block, state: &mut EvmState) -> Result<Vec<Receipt>, EvmError> {
    ...
    let mut cummulative_gas_used = 0;
    for transaction in block.body.transactions.iter() {
        let result = execute_tx(transaction, block_header, state, spec_id)?;
        cummulative_gas_used += result.gas_used();
        let receipt = Receipt::new(transaction.tx_type(), result.is_success(), cumulative_gas_used, result.logs());
...
/// When the basefee tracking is disabled (ie. env.disable_base_fee = true; env.disable_block_gas_limit = true;)
...
fn adjust_disabled_base_fee(block_env: &mut BlockEnv, tx_gas_price: Uint<256, 4>, tx_blob_gas_price: Option<Uint<256, 4>>) {
...
pub fn process_withdrawals(state: &mut EvmState, withdrawals: &[Withdrawal]) -> Result<(), StoreError> {
...
pub fn spec_id(store: &Store, block_timestamp: u64) -> Result<SpecId, StoreError> {
    Ok(
        if store.get_cancun_time()?.is_some_and(|t| t <= block_timestamp) {
            SpecId::CANCUN
        } else if store.get_shanghai_time()?.is_some_and(|t| t <= block_timestamp) {
            SpecId::SHANGHAI
        } else {
            SpecId::MERGE
        },
