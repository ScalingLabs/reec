use reec_core::{Address as CoreAddress, H256 as CoreH256};
...
impl revm::Database for StoreWrapper {
    ...
    fn basic(&mut self, address: RevmAddress) -> Result<Option<RevmAccountInfo>, Self::Error> {
        let acc_info = match self.0.get_account_info(CoreAddress::from(address.0.as_ref()))?
        {
            None => return Ok(None),
            Some(acc_info) => acc_info,
        };
        let code = self.0.get_account_code(acc_info.code_hash)?.map(|b| RevmBytecode::new_raw(RevmBytes(b)));
    ...
    fn code_by_hash(&mut self, code_hash: RevmB256) -> Result<RevmBytecode, Self::Error> {
        self.0.get_account_code(CoreH256::from(code_hash.as_ref()))?
