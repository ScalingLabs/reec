...
use reec_core::{
    H512, rlp::{
        decode, encode::RLPEncode as _, error::RLPDecodeError, structs::{Decoder, Encoder}
    }
};
use k256::PublicKey;
...
impl HelloMessage {
    pub fn new(capabilites: Vec<(String, u8)>, node_id: PublicKey) -> Self {
...
impl RLPxMessage for HelloMessage {
    ...
    fn decode(msg_data: &[u8]) -> Result<Self, RLPDecodeError> {
        ...
        let (capabilities, decoder): (Vec<(String, u8)>, _) = decoder.decode_field("capabilites").unwrap();
        ...
        // Implementaions muts ignore any additional list elements
...
impl RLPxMessage for DisconnectMessage {
    fn encode(&self, buf: &mut dyn BufMut) {
        ...
        // Disconnect msg_data is readon or none
        match self.reason {
            Some(value) => Encoder::new(&mut encoded_data).encode_field(&value).finish(),
        ...
        let compressed_size = snappy_encoder.compress(&encoded_data, &mut msg_data).unwrap();
...
impl RLPxMessage for PongMessage {
    ...
    fn decode(msg_data: &[u8]) -> Result<Self, RLPDecodeError> {
        // decode pong message: data is empty list [] buf it is snappy compressed
        let mut snappy_decoder = SnappyDecoder::new();
        let decompressed_data = snappy_decoder.decompress_vec(msg_data).unwrap();
        let decoder = Decoder::new(&decompressed_data)?;
        let result = decoder.finish_unchecked();
