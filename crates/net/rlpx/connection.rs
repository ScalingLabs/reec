...
pub(crate) type Aes128Ctr64BE = ctr::Ctr64BE<aes::Aes256>;
...
impl<'a> RLPxConnectionPending {
    ...
    pub async fn receive_hello<S: AsyncRead>(self, stream: S) -> RLPxConnection {
        ...
        // Bot are padded to the block's size (16 bytes)
        ...
        let frame_size: usize = u32::from_be_bytes([0, header_text[0], header_text[1], header_text[2]]).try_into().unwrap();
        ...
        let (_capabilities, decoder): (Vec<(Bytes, u64)>, _) = decoder.decode_field("capabilities").unwrap();
        // TODO: derive shared capabilities for furter communication
