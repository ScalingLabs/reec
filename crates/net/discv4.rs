...
use reec_core::{H256, H512, H520};
use k256::ecdsa::{RecoveryId, Signature, SigningKey, VerifyingKey};
use sha3::{Digest, Keccak256};

pub fn get_expiration(seconds: u64) -> u64 {
    (SystemTime::now() + Duration::from_secs(seconds)).duration_since(UNIX_EPOCH).unwrap().as_secs()
}
...
pub fn is_expired(expiration: u64) -> bool {
    ...
    (expiration as i64) < SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs() as i64
}
...
impl Packet {
    pub fn decode(encoded_packet: &[u8]) -> Result<Packet, PacketDecodeErr> {
        // The packet structure is hash || signature || packet-type || packet-data
        let hash_len = 32;
        let signature_len = 65;
        let header_size = hash_len + signature_len;
        ...
        let peer_pk = VerifyingKey::recover_from_prehash(&digest, signature, rid).map_err(|_| PacketDecodeErr::InvalidSignature)?;
        ...
        let message = Message::decode_with_type(packet_type, &encoded_msg[1..]).map_err(PacketDecodeErr::RLPDecodeError)?;
...
pub(crate) enum Message {
    /// A Ping message should be responded with a Pong message
    Ping(PingMessage),
    Pong(PongMessage),
...
impl Message {
    pub fn encode_with_header(&self, buf: &mut dyn BufMut, node_signer: &SigningKey) {
        ...
        let (signature, recovery_id) = node_signer.sign_prehash_recoverable(&digest).expect("failed to sign");
    ...
    pub fn decode_with_type(packet_type: u8, msg: &[u8]) -> Result<Message, RLPDecodeError> {
        // NOTE: extra elements inside the message should be ignored, along with extra data after the message.
        match packet_type {
            0x01 => {
            ...
            0x06 => {
                let (enr_response_msg, _rest) = ENRRequestMessage::decode_unfinished(msg)?;
...
pub(crate) struct PingMessage {
    /// The Ping message version. Should be set to 4, but mustn't be enforeced.
    version: u8,
    /// The endpoint of the sender
    pub from: Endpoint,
    /// The endpoint of the receiver
    pub to: Endpoint,
    /// The expiration time of the message. If the message is older than this time, it shouldn't be responded to.
    pub expiration: u64,
...
impl PingMessage {
    pub fn new(from: Endpoint, to: Endpoint, expiration: u64) -> Self {
        Self { version: 4, from, to, expiration, enr_seq: None }
    }

    #[allow(unused)]
    pub fn with_enr_seq(self, enr_seq: u64) -> Self {
        Self { enr_seq: Some(enr_seq),
            ..self
        }
    }
}
...
pub(crate) struct FindNodeMessage {
    ...
    pub target: H512,
    /// The expiration time of the message. If the message is older than ths time, it should't be responded to.
    pub expiration: u64,
}
...
pub struct FindNodeRequest {
    ...
    /// We keep track of this number since we will accept neighbour messages until the max_per_bucket
...
pub(crate) struct PongMessage {
    /// The endpoint of the receiver
    pub to: Endpoint,
    /// The hash of the corresponging ping packet.
    pub ping_hash: H256,
    /// The expiration time of the message. If the message is older than this time, it shouldn't be responded to.
    pub expiration: u64,
    /// The ENR sequence number of the sender. This field is optional.
...
mod tests {
    ...
    fn test_encode_pong_message_with_enr_seq() {
        ...
        let ping_hash: H256 = H256::from_str("3e1bf98f025f98d54ed2f61bbef63b6b46f50e12d7b937d6bdea19afd640be23").unwrap();
    ...
    fn test_decode_neighbors_message() {
        ...
        let expected = Message::Neighbors(NeighborsMessage::new(nodes, expiration));
        assert_eq!(decoded, expected);
    }
