...
impl KademliaTable {
    ...
    //         // TODO: revalidate least recently seen node as described in <https://github.com/ethereum/devp2p/blob/master/discv4.md#kademlia-table>
    //         bucket.pop();
    //     }
    //     bucket.push(peer)
    // }
    ...
    pub fn get_by_node_id_mut(&mut self, node_id: H512) -> Option<&mut PeerData> {
        ...
        bucket.iter_mut().find(|entry| entry.node.node_id == node_id)
    }
    ...
    fn remove_from_replacements(&mut self, node_id: H512) {
        self.replacements = self.replacements.drain(..).filter(|r| r.node.node_id != node_id).collect();
    }
