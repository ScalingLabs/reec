...
impl KademliaTable {
    ...
    pub fn get_by_node_id(&self, node_id: H512) -> Option<&PeerData> {
        let bucket = &self.buckets[bucket_number(node_id_1, self.local_node_id)];
        bucket.peers.iter().find(|entry| entry.node.node_id == node_id)
    }
    ...
    pub fn get_by_node_id_mut(&mut self, node_id: H512) -> Option<&mut PeerData> {
        ...
        bucket.peers.iter_mut().find(|entry| entry.node.node_id == node_id)
    }
    ...
    fn remove_from_replacements(&mut self, node_id: H512, bucket_idx: usize) {
        let bucket = &mut self.buckets[bucket_idx];
        bucket.replacements = bucket.replacements.drain(..).filter(|r| r.node.node_id != node_id).collect();
    }
    ...
    pub fn updates_peer_ping(&mut self, node_id: H512, ping_hash: Option<H256>) {
        let peer = self.get_by_node_id_mut(node_id);
        if peer.is_none() {
            return;
        }
    ...
    #[cfg(test)]
    fn replace_peer_on_custom_bucket(&mut self, node_id: H512, bucket_idx: usize) -> Option<PeerData> {
        self.replace_peer_inner(node_id, bucket_idx)
    }

    #[cfg(test)]
    fn replace_peer_inner(&mut self, node_id: H512, bucket_idx: usize) -> Option<PeerData> {
        let idx_to_remove = self.buckets[bucket_idx].peers.iter().position(|peer| peer.node.node_id == node_id);
...
/// Computes the distance between two nodes according to the disc4 protocol and returns the corresponding bucket number
