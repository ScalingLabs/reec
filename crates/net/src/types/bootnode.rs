...
impl FromStr for BootNode {
    ...
    /// Takes a str with the format "enode://<node ID>@<IP address>:<port>" and parses it to a Bootnode
    fn from_str(input: &str) -> Result<BootNode, ParseIntError> {
        // TODO: error handling
        let node_id = H512::from_str(&input[8..136]).expect("Failed to parse node id");
        let socket_address: SocketAddr = input[137..].parse().expect("Failed to parse bootnode address and port");
        Ok(
            BootNode {
                node_id,
                socket_address,
...
pub fn decode_hex(s: &str) -> Result<Vec<u8>, ParseIntError> {
    (0..s.len())
        .step_by(2)
        .map(|i| u8::from_str_radix(&s[i..i + 2], 16)).collect()
}
