...
impl RLPDecode for NodeRecord {
    fn decode_unfinished(rlp: &[u8]) -> Result<(Self, &[u8]), RLPDecodeError> {
        ...
        if let Some((_key, id)) = id_pair {
            ...
            Err(RLPDecodeError::Custom("Invalid node record, 'id field missing".into()))
        }
    }
}
...
/// Thisfunction returns a vector with (key, value) tuples. Both keys and values are stored as Bytes.
/// Each value is the acutal RLP encoding of the field including its prefix so it can be decoded as T::decode(value)
