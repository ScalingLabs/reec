...
impl RLPxConnection {
    ...
    pub fn encode_auth_message(
        ...
        // Pad with random amount of data. The amount needs to be at least 100 bytes to make the message distinguishable from pre-EIP-8 handshakes.
        ...
        let ecies_overhead = SIGNATUE_SIZE + IV_SIZE + MAC_FOOTER_SIZE;
        let auth_size: u16 = (encoded_auth_msg.len() + ecies_overhead).try_into().unwrap();
        ...
        aes_cipher.try_apply_keystream(&mut encoded_auth_msg).unwrap();
    ...
    fn sign_shared_secret(&self, shared_secret: H256) -> H520 {
        ...
        let (signature, rid) = SigningKey::from(&self.ephemeral_key).sign_prehash_recoverable(&signature_prehash.0).unwrap();
    ...
    pub fn decode_ack_message(
        ...
        // Decrypt the message withh the AES key
    ...
    fn derive_secrets(&self, ack: AckMessage) -> HandshakeData {
        // TODO: let's not panic
        let ephemeral_key_secret = ecdh_xchng(&self.ephemeral_key, &ack.get_ephemeral_pubkey().unwrap());
...
pub(crate) struct AckMessage {
    ...
    pub nonce: H256,
    /// The verion of RLPx used by the recipeint.
    /// The current verison is 5.
    pub version: u8,
}
...
fn ecdh_xchng(secret_key: &SecretKey, public_key: &PublicKey) -> [u8; 32] {
    k256::ecdh::diffie_hellman(secret_key.to_nonzero_scalar(), public_key.as_affine()).raw_secret_bytes()[..32].try_into().unwrap()
}

fn kdf(secret_key: &[u8], output: &mut [u8]) {
...
pub fn pubkey2id(pd: &PublicKey) -> H512 {
