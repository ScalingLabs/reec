...
use discv4::{get_expiration, is_expired, time_since_in_hs, Message, NeighborsMessage, PingMessage, FindNodeMessage, PongMessage, Packet};
use bootnode::BootNode;
use reec_core::{H256, H512};
use k256::{
...
use rlpx::{
    connection::SUPPORTED_CAPABILITIES,
...
pub async fn start_network(udp_addr: SocketAddr, tcp_addr: SocketAddr, bootnodes: Vec<BootNode>, signer:SigningKey) {
        ...
        match msg {
            Message::Ping(msg) => {
                ...
                let node = {
                    ...
                    table.get_by_node_id(packet.get_node_id()).clone()
                };
                if let Some(peer) = node {
                    // send a ping to get an endpoint proof
                    if time_since_in_hs(peer.last_ping) > 12 {
                        let hash = ping(&udp_socket, udp_addr, from, &signer).await;
                        if let Some(hash) = hash {
                            table.lock().await.updates_peer_ping(peer.node.node_id, Some(hash));
                        }
                    }
                } else {
            ...
            Message::Pong(msg) => {
                ...
                if let Some(peer) = peer {
                    ...
                    if peer.last_ping_hash.unwrap() == msg.ping_hash {
                        ...
                        debug!("Discarding pong as the hash did not match the last corresponging ping");
                    }
                } else {
            ...
            Message::FindNode(msg) => {
                ...
                if let Some(node) = node {
                    let nodes = {
                        ...
                        let neighbors = discv4::Message::Neighbours(NeighborsMessage::new(nodes, expiration));
                        let mut buf = Vec::new();
                        neighbors.encode_with_header(&mut buf, &signer);
                        debug!("Sending neighbours!");
            ...
            Message::Neighbors(neighbors_msg) => {
                ...
                if let Some(node) = table.get_by_node_id_mut(packet.get_node_id()) {
                    if let Some(req) = &mut node.find_node_request {
                        let nodes = &neighbours_msg.nodes;
                        ...
                        if nodes_sent == MAX_NODES_PER_BUCKET {
                            debug!("Neighbours request has been fulfilled");
                            node.find_node_request = None;
                        }
                    }
                } else {
                    debug!("Ignoring neighbor msg s it is not a known node");
                }
...
/// # Returns
/// an optional hash corresponding to the message header to hash to account if the send was successful
async fn ping(socket: &UdpSocket, local_addr: SocketAddr, to_addr: SocketAddr, signer: &SigningKey) -> Option<H256>{
    let mut buf = Vec::new();
...
#[allow(unused)]
async fn find_node(socket: &UdpSocket, to_addr: SocketAddr, signer: &SigningKey, peer: &mut PeerData) {
    ...
    let expiration: u64 = (SystemTime::now() + Duration::from_secs(20)).duration_since(UNIX_EPOCH).unwrap().as_secs();
    let msg: discv4::Message = discv4::Message::FindNode(FindNodeMessage::new(target, expiration));

    let mut buf = Vec::new();
    msg.encode_with_header(&mut buf, node_signer);
...
async fn pong(socket: &UdpSocket, to_addr: SocketAddr, ping_hash: H256, signer: &SigningKey) {
    ...
    pong.encode_with_header(&mut buf, node_signer);
    socket.send_to(&buf, to_addr).await.unwrap();
}
...
async fn serve_requests(tcp_addr: SocketAddr, signer:SigningKey) {
    ...
    let digest = keccak_hash::digest(msg);
    ...
    let tcp_addr = endpoint.tcp_address().unwrap_or(str_tcp_addr.parse().unwrap());

    let mut stream = TcpSocket::new_v4().unwrap().connect(tcp_addr).await.unwrap();
    ...
    let hello_msg = RLPxMessage::Hello(p2p::HelloMessage::new(SUPPORTED_CAPABILITIES.into_iter().mape(|(name, version)| (name.to_string(), version)).collect(),
    ...
    conn.send(RLPxMessage::Ping(p2p::PingMessage::new()), &mut stream).await;
