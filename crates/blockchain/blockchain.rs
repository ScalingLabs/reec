...
use reec_storage::{AccountUpdate, Store};
use reec_vm::{evm_state, execute_block, spec_id, EvmState, SpecId};
...
pub fn add_block(block: &Block, storage: &Store) -> Result<(), ChainError> {
    ...
    let (receipts, account_updates): (Vec<Receipt>, Vec<AccountUpdate>) = {
        ...
        #[cfg(not(feature = "levm"))]
        {
            ...
            let account_updates = reec_vm::get_state_transitions(&mut state);
