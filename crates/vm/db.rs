use reec_core::{types::BlockHash, Address as CoreAddress, H256 as CoreH256};
...
cfg_if::cfg_if! {
    if #[cfg(feature = "levm")] {
        use reec_core::{U256 as CoreU256};
        use reec_levm::db::Database as LevmDatabase;

        impl LevmDatabase for StoreWrapper {
            fn get_account_info(&self, address: CoreAddress) -> reec_levm::account::AccountInfo {
                let acc_info = self
                    .store
