use crate::{error::StoreError, trie::node_ref};
...
/// Only node hashes for root nodes computed with  `Trie::compute_hash` are stored
...
table!(
    ...
    /// Stores root nodes which hashes have been computed by `compute_hash`
...
impl TrieDB {
    ...
    pub fn create(trie_dir: &str) -> Result<TrieDB, StoreError> {
        let tables = [table_info!(Nodes), table_info!(RootNodes)].into_iter().collect();
    ...
    pub fn get_node(&self, node_ref: NodeRef) -> Result<Option<Node>, StoreError> {
        self.read::<Nodes>(node_ref)?.map(|rlp| Node::decode(&rlp).map_err(StoreError::RLPDecode)).transpose()
    }
    ...
    fn write<T: libmdbx::orm::Table>(&self, key: T::Key, value: T::Value) -> Result<(), StoreError> {
...
fn last_node_ref(db: &Database) -> Result<Option<NodeRef>, StoreError> {
    ...
    Ok(cursor.last().map_err(StoreError::LibmdbxError)?.map(|(node_ref, _)| node_ref))
}