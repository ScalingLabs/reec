...
impl<'a> AsRef<[u8]> for NodeHashRef {
    fn as_ref(&self) -> &[u8] {
        match self {
...
impl<'a> NodeHasher<'a> {
    ...
    fn write_len(&mut self, short_base: u8, long_base: u8, value_len: usize) {
        match value {
...
impl RLPEncode for NodeHash {
    fn encode(&self, buf: &mut dyn bytes::BufMut) {
        ...
        Encoder::new(buf).encode_field(&length).encode_field(&hash).finish();
    }
}
