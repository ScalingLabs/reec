...
use bytes::Bytes;
use reec_core::types::{AccountInfo, AccountState, Block, BlockHash, BlockBody, BlockHeader, BlockNumber, ChainConfig, code_hash, Genesis, GenesisAccount, Index, Receipt, Transaction};
...
impl AccountUpdate {
    /// Creates a new empty update for the given account
    ...
    /// Creates a new update representing an account removal
...
impl Store {
    ...
    pub fn add_transaction_location(
        ...
        self.engine.lock().unwrap().add_transaction_location(transaction_hash, block_number, block_hash, index)
    }
    ...
    pub fn get_nonce_by_account_address(&self, block_number: BlockNumber, address: Address) -> Result<Option<u64>, StoreError> {
        let Some(state_trie) = self.engine.lock().unwrap().state_trie(block_number)? else {
            return Ok(None);
        };
        let hashed_address = hash_address(&address);
        ...
        for update in account_updates.iter() {
            ...
            if update.removed {
                ...
                if !update.added_storage.is_empty() {
                    ...
                    account_state.storage_root = reec_core::types::compute_storage_root(
    ...
    pub fn setup_genesis_state_trie(
        ...
        for (address, account) in genesis_accounts {
            ...
            let storage_root = reec_core::types::compute_storage_root(&account.storage);
    ...
    pub fn add_block(&self, block: Block) -> Result<(), StoreError> {
        ...
        self.add_transaction_locations(&block.body.transactions, block_number, hash)?;
...
fn hash_address(address: &Address) -> Vec<u8> {
        Keccak256::new_with_prefix(address.to_fixed_bytes()).finalize().to_vec()
}
...
mod tests {
    ...
    fn test_store_transaction_location(store: Store) {
        ...
        let block_number = 6;
        let index = 3;
        store.add_transaction_location(transaction_hash, block_number, block_hash, index).unwrap();
