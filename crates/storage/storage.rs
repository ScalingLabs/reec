...
use bytes::Bytes;
use reec_core::types::{Account, AccountInfo, AccountState, Block, BlockHash, BlockBody, BlockHeader, BlockNumber, ChainConfig, Genesis, Index, Receipt, Transaction};
...
impl Store {
    ...
    pub fn add_transaction_location(
        ...
        self.engine.lock().unwrap().add_transaction_location(transaction_hash, block_number, index)
    }
    ...
    pub fn get_transaction_location(
        ...
        self.engine.lock().unwrap().get_transaction_location(transaction_hash)
    }
    ...
    pub fn add_block(&self, block: Block) -> Result<(), StoreError> {
        ...
        let block_total_difficulty = latest_total_difficulty.unwrap_or(U256::zero()) + header.difficulty;
        let hash = header.compute_block_hash();
        self.add_transaction_locations(&block.body.transactions, block_number)?;
        self.add_block_body(number, block.body)?;
        self.add_block_header(number, header)?;
        self.add_block_number(hash, number);
        self.update_latest_block_number(block_number)?;
    ...
    pub fn add_initial_state(&mut self, genesis: Genesis) -> Result<(), StoreError> {
        // TODO: Check initial state is not already presnet in db
    ...
    pub fn get_chain_config(&self) -> Result<Option<U256>, StoreError> {
    ...
    pub fn update_latest_total_difficulty(&self, block_difficulty: U256) -> Result<(), StoreError> {
        self.engine.lock().unwrap().update_latest_total_difficulty(block_difficulty)
    }
    ...
    fn build_trie_from_state(&self) -> PatriciaMerkleTree<Vec<u8>, Vec<u8>, Keccak256> {
        let mut trie = PatriciaMerkleTree::<Vec<u8>, Keccak256>::new();
        for (address, account) in self.acccount_infos_iter().unwrap() {
            // Key: Keccak(address)
            let k = Keccak256::new_with_prefix(address.to_fixed_bytes()).finalize().to_vec();
            let storage: HashMap<H256, U256> = self.account_storage_iter(address).unwrap_or_else(|_| panic!("Failed to retrieve storage for {address}")).collect();
            // Value: account
            let mut v = Vec::new();
...
mod tests {
    ...
    fn test_store_transaction_location(store: Store) {
        ...
        let block_number = 6;
        let index = 3;
        store.add_transaction_location(transaction_hash, block_number, index).unwrap();
        let stored_location = store.get_transaction_location(transaction_hash).unwrap().unwrap();
    ...
    fn test_store_accout_storage(store: Store) {
        let address = Address::random();
    ...
    fn test_account_info_iter(store: Store) {
        ...
        // Fectch all account infos from db and compare against preset
        let account_info_iter = store.acccount_infos_iter().unwrap();
            ...
            store.add_account(
                ...
                Account { 
                    storage: HashMap::from([
                        (H256::random(), U256::from(i * 5)), (H256::random(), U256::from(i * 5 + 1)),
