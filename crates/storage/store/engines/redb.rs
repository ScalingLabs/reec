...
use reec_core::types::BlockBody;
use reec_core::{
    types::{BlobsBundle, Block, BlockHash, BlockHeader, BlockNumber, ChainConfig, Index, Receipt},
    H256, U256,
};
use reec_rlp::decode::RLPDecode;
use reec_rlp::encode::RLPEncode;
use reec_trie::{
...
impl StoreEngine for RedBStore {
    ...
    fn add_block_total_difficulty(
        &self,
        block_hash: BlockHash,
        block_total_difficulty: reec_core::U256,
    ) -> Result<(), StoreError> {
    ...
    fn get_block_total_difficulty(
        &self,
        block_hash: BlockHash,
    ) -> Result<Option<reec_core::U256>, StoreError> {
        Ok(self
            .read(
                BLOCK_TOTAL_DIFFICULTIES_TABLE,
                <H256 as Into<BlockHashRLP>>::into(block_hash),
    ...
    fn add_transaction_location(
        &self,
        transaction_hash: reec_core::H256,
        block_number: BlockNumber,
        block_hash: BlockHash,
        index: Index,
    ) -> Result<(), StoreError> {
    ...
    fn update_latest_total_difficulty(
        &self,
        latest_total_difficulty: reec_core::U256,
    ) -> Result<(), StoreError> {
        self.write(
            CHAIN_DATA_TABLE,
    ...
    fn get_latest_total_difficulty(&self) -> Result<Option<reec_core::U256>, StoreError> {
    ...
    fn open_storage_trie(
        &self,
        hashed_address: reec_core::H256,
        storage_root: reec_core::H256,
    ) -> reec_trie::Trie {
        let db = Box::new(RedBMultiTableTrieDB::new(self.db.clone(), hashed_address.0));
        Trie::open(db, storage_root)
    }

    fn open_state_trie(&self, state_root: reec_core::H256) -> reec_trie::Trie {
...
pub fn init_db() -> Result<Database, StoreError> {
    let db = Database::create("reec.redb")?;
