...
impl StoreEngine for Store {
    fn add_account_info(
        ...
        // Write account to mdbx
            ...
            txn.upsert::<AccountInfos>(address.into(), account_info.into()).map_err(StoreError::LibmdbxError)?;
            txn.commit().map_err(StoreError::LibmdbxError)
    }

    fn get_account_info(&self, address: Address) -> std::result::Result<Option<AccountInfo>, StoreError> {
        // Read account from mdbx
        let txn = self.db.begin_read().map_err(StoreError::LibmdbxError)?;
        Ok(txn.get::<AccountInfos>(address.into()).map_err(StoreError::LibmdbxError)?.map(|a| a.to()))
    }
    ...
    fn add_block_header(
        ...
        txn.upsert::<Headers>(block_number, block_header.into()).map_err(StoreError::LibmdbxError)?;
        txn.commit().map_err(StoreError::LibmdbxError)
    }
    ...
    fn add_block_body(
        ...
        let txn = self.db.begin_readwrite().map_err(StoreError::LibmdbxError)?;
        txn.upsert::<Bodies>(block_number, block_body.into()).map_err(StoreError::LibmdbxError)?;
        txn.commit().map_err(StoreError::LibmdbxError)
    }
    ...
    fn add_block_number(
        ...
        let txn = self.db.begin_readwrite().map_err(StoreError::LibmdbxError)?;
        txn.upsert::<BlockNumbers>(block_hash.into(), block_number).map_err(StoreError::LibmdbxError)?;
        txn.commit().map_err(StoreError::LibmdbxError)
    }
    ...
    fn add_storage_at(
        ...
        let txn = self.db.begin_readwrite().map_err(StoreError::LibmdbxError)?;
        txn.upsert::<AccountStorages>(address.into(), (storage_key.into(), storage_value.into())).map_err(StoreError::LibmdbxError)?;
        txn.commit().map_err(StoreError::LibmdbxError)
    }
