...
use bytes::Bytes;
use reec_core::types::{AccountInfo, BlockBody, BlockHash, BlockHeader, BlockNumber};
...
enum StoreCommand {
    ...
    Get(DbSelector, Key, SyncSender<Result<Option<Value>, StoreError>>)
}
...
impl Store {
    pub fn new(path: &str) -> Result<Self, StoreError> {
        ...
        let (command_sender, command_receiver): (Sender<StoreCommand>, Receiver<StoreCommand>) = channel();
        thread::spawn(move || {
