...
use bytes::Bytes;
use reec_core::types::{AccountInfo, BlockBody, BlockHash, BlockHeader, BlockNumber, ChainConfig, Index, Receipt};
...
impl StoreEngine for Store {
    ...
    fn account_infos_iter(&self) -> Result<Box<dyn Iterator<Item = (Address, AccountInfo)>>, StoreError> {
    ...
    fn add_block_total_difficulty(&mut self, block_hash: BlockHash, block_total_difficulty: U256) -> Result<(), StoreError> {
        self.block_total_difficulties.insert(block_hash, block_total_difficulty);
        Ok(())
    }

    fn get_block_total_difficulty(&self, block_hash: BlockHash) -> Result<Option<U256>, StoreError> {
    ...
    fn add_transaction_location(
        ...
        self.transaction_locations.insert(transaction_hash, (block_number, index));
        Ok(())
    }
    ...
    fn get_receipt(
        ...
        Ok(self.receipts.get(&block_number).and_then(|entry| entry.get(&index)).cloned())
    }
    ...
    fn get_storage_at(
        ...
        Ok(self.account_storages.get(&address).and_then(|entry| entry.get(&storage_key).cloned()))
    }
    ...
    fn account_storage_iter(&mut self, address: Address) -> Result<Box<dyn Iterator<Item = (H256, U256)>>, StoreError> {
        Ok(Box::new(
            self.account_storages.get(&address).cloned().into_iter().flatten()
        ))
    }
    ...
    fn get_chain_config(&self) -> Result<Option<U256>, StoreError> {
    ...
    fn update_latest_total_difficulty(&mut self, latest_total_difficulty: U256) -> Result<(), StoreError> {
        self.chain_data.latest_total_difficulty.replace(latest_total_difficulty);
        Ok(())
    }
