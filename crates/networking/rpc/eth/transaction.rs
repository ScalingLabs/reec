...
use reec_vm::{evm_state, ExecutionResult, SpecId};
use serde::Serialize;
...
impl RpcHandler for CreateAccessListRequest {
    ...
    fn handle(&self, context: RpcApiContext) -> Result<Value, RpcErr> {
        ...
        let (gas_used, access_list, error) = match reec_vm::create_access_list(
            &self.transaction,
            &header,
...
impl RpcHandler for EstimateGasRequest {
    ...
    fn handle(&self, context: RpcApiContext) -> Result<Value, RpcErr> {
        ...
        let spec_id = reec_vm::spec_id(&storage.get_chain_config()?, block_header.timestamp);
...
fn simulate_tx(
    ...
    match reec_vm::simulate_tx_from_generic(
        transaction,
        block_header,
