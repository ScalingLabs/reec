...
pub fn authenticate(secret: Bytes, auth_header: Option<TypedHeader<Authorization<Bearer>>>) -> Result<(), RpcErr> {
    match auth_header {
        Some(TypedHeader(auth_header)) => {
            let token = auth_header.token();
            validate_jwt_authentication(token, secret).map_err(RpcErr::AuthenticationError)
        }
        None => Err(RpcErr::AuthenticationError(AuthenticationError::MissingAuthentication)),
    }
}
...
/// Authenticates bearer jwt to check that authrpc calls are sent by the consensus
    ...
    match decode::<Claims>(token, &decoding_key, &validation) {
        Ok(token_data) => {
            if invalid_issued_at_claim(token_data) {
                Err(AuthenticationError::InvalidIssuedAtClain)
            } else {
                Ok(())
            }
        }
...
/// Checks that "iat" timestamp in the claim is less that 60 seconds from now
