...
impl FullBlockBody {
    pub fn from_body(body: BlockBody, block_number: BlockNumber, block_hash: BlockHash) -> FullBlockBody {
        let mut transactions = Vec::new();
        for (index, tx) in body.transactions.iter().enumerate() {
            transactions.push(RpcTransaction::build(tx.clone(), block_number, block_hash, index));
        }
        FullBlockBody { 
            transactions, 
            uncles: body.ommers, 
