...
pub fn new_payload_v3(request: NewPayloadV3Request, storage: Store) -> Result<PayloadStatus, RpcErr> {
    ...
    // As we don't currently handle blob txs, we just check that it is empty
    ...
    match add_block(&block, storage.clone()) {
        ...
        Err(ChainError::ParentNotFound) => Ok(PayloadStatus::invalid_with_err("Could not reference parent block with parent_hash",)),
