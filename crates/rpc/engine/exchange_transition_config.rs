use reec_core::{serde_utils, H256};
...
#[derive(Debug)]
pub struct ExchangeTransitionConfigV1Reg {
    payload: ExchangeTransitionConfigPayload,
}

impl std::fmt::Display for ExchangeTransitionConfigV1Reg {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f, "ExchangeTransitionConfigV1Req {{ terminal_total_difficulty: {}, terminal_block_hash: {:?}, terminal_block_number: {} }}", 
...
impl RpcHandler for ExchangeTransitionConfigV1Reg {
    fn parse(params: &Option<Vec<Value>>) -> Option<Self> {
        let params = params.as_ref()?;
        if params.len() != 1 {
            return None;
        };
        let payload: ExchangeTransitionConfigPayload = serde_json::from_value(params[0].clone()).unwrap();
        Some(ExchangeTransitionConfigV1Reg { payload })
    }
    ...
    fn handle(&self, storage: Store) -> Result<Value, RpcErr> {
        ...
        if terminal_total_difficulty.unwrap_or_default() != payload.terminal_total_difficulty {
            warn!("Invalid terminal total difficulty configured: execution {:?} consensus {}", terminal_total_difficulty, payload.terminal_total_difficulty);
        };
