...
pub fn new_payload_v3(request: NewPayloadV3Request, storage: Store) -> Result<PayloadStatus, RpcErr> {
    ...
    // Payload Validation
    // Check timestamp does not fall within the time frame of the Cancum fork
    ...
    // Concatenate blob versioned hashes lists (tx.blob_versioned_hashed) of each blob transaction included in the payload, respecting the order of inclusion
    // and check that the resulting array matches expected_blob_versioned_hashes
    let blob_versioned_hashes: Vec<H256> = block.body.transactions.iter().flat_map(|tx| tx.blob_versioned_hashes()).collect();
