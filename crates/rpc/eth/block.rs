...
use crate::{
    types::{
        ...
        receipt::{RpcReceipt, RpcReceiptBlockInfo, RpcReiptTxInfo}, 
    },
    utils::RpcErr,
    RpcHandler,
};
use reec_core::{
    ...
    types::{calculate_base_fee_per_blob_gas, Block, BlockBody, BlockHash, BlockHeader, BlockNumber, Receipt},
};
...
#[derive(Clone, Debug)]
pub struct GetRwaHeaderRequest {
    pub block: BlockIdentifier,
}
...
impl RpcHandler for GetBlockTransactionCountRequest {
    ...
    fn handle(&self, storage: Store) -> Result<Value, RpcErr> {
        info!("Requested transaction count for block with number: {}", self.block);
        let block_number = match self.block.resolve_block_number(&storage)? {
            Some(block_number) => block_number,
            _ => return Ok(Value::Null),
        };
...
impl RpcHandler for GetRwaHeaderRequest {
    fn parse(params: &Option<Vec<Value>>) -> Result<GetRwaHeaderRequest, RpcErr> {
        let params = params.as_ref().ok_or(RpcErr::BadParams)?;
        if params.len() != 1 {
            return Err(RpcErr::BadParams);
        };
        Ok(GetRwaHeaderRequest { block: BlockIdentifier::parse(params[0].clone(), 0)?, })
    }
...
impl RpcHandler for GetRawReceipts {
    ...
    fn handle(&self, storage: Store) -> Result<Value, RpcErr> {
        ...
        let receipts: Vec<String> = get_all_block_receipts(block_number, header, body, &storage)?.iter().map(|receipt| format!("0x{}", hex::encode(receipt.encode_to_vec()))).collect();
        serde_json::to_value(receipts).map_err(|_| RpcErr::Internal)
    }
}

pub fn get_all_block_rpc_receipts(block_number: BlockNumber, header: BlockHeader, body: BlockBody, storage: &Store) -> Result<Vec<RpcReceipt>, RpcErr> {
    ...
    for (index, tx) in body.transactions.iter().enumerate() {
        ...
        let tx_info = RpcReceiptTxInfo::from_transaction(tx.clone(), index, gas_used, blob_gas_price);
...
pub fn get_all_block_receipts(block_number: BlockNumber, header: BlockHeader, body: BlockBody, storage: &Store) -> Result<Vec<Receipt>, RpcErr> {
    let mut receipts = Vec::new();
    // Check if this is the genesis block
    if header.parent_hash.is_zero() {
        return Ok(receipts);
