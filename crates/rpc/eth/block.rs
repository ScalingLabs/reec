...
use reec_core::{
    types::{
    AccessListEntry, BlockHash, BlockNumber, BlockSerializable, GenericTransaction, ReceiptWithTxAndBlockInfo},
    H256,
};
...
impl GetTransactionByBlockNumberAndIndexRequest {
    pub fn parse(params: &Option<Vec<Value>>) -> Option<GetTransactionByBlockNumberAndIndexRequest> {
        let params = params.as_ref()?;
        if params.len() != 2 {
            return None;
        };
...
pub fn get_block_receipts(
    ...
    // Fetch receipt for each tx in the block and block and tx info
...
pub fn get_transaction_by_hash(
    ...
    info!("Requested transaction with hash: {}",
