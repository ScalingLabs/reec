...
impl RpcHandler for GetTransactionByHashRequest {
    ...
    fn handle(&self, storage: Store) -> Result<Value, RpcErr> {
        ...
        let transaction: reec_core::types::Transaction = match storage.get_transaction_by_hash(self.transaction_hash)? {
...
impl RpcHandler for GetTransactionReceiptRequest {
    ...
    fn handle(&self, storage: Store) -> Result<Value, RpcErr> {
        ...
        let (block_number, index) = match storage.get_transaction_location(transaction_hash)? {
            Some(location) => location,
            _ => return Ok(Value::Null),
        };
        let block_header = match storage.get_block_header(block_number)? {
