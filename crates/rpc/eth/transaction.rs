...
pub struct AccessListResult {
    ...
    error: Option<String>,
    #[serde(with = "reec_core::serde_utils::u64::hex_str")]
    gas_used: u64
}
...
impl RpcHandler for CallRequest {
    ...
    fn handle(&self, storage: Store) -> Result<Value, RpcErr> {
        ...
        // Run transaction
        let data = match reec_evm::simulate_tx_from_generic(&self.transaction, &header, &mut evm_state(storage), SpecId::CANCUN)? {
            ...
            ExecutionResult::Revert { gas_used:_, output, } => {
                return Err(RpcErr::Revert { data: format!("0x{:#x}", output), });
            }
            ExecutionResult::Halt { reason: _, gas_used: _ } => Bytes::new(),
        };
...
impl RpcHandler for GetTransactionByBlockNumberAndIndexRequest {
    fn parse(params: &Option<Vec<Value>>) -> Result<GetTransactionByBlockNumberAndIndexRequest, RpcErr> {
        let params = params.as_ref().ok_or(RpcErr::BadParams)?;
        if params.len() != 2 {
            return Err(RpcErr::BadParams);
        };
        ...
        Ok(GetTransactionByBlockNumberAndIndexRequest { 
            ...
            transaction_index: usize::from_str_radix(index_as_string.trim_start_matches("0x"), 16).map_err(|_| RpcErr::BadParams)?,
        })
    }

    fn handle(&self, storage: Store) -> Result<Value, RpcErr> {
        info!("Requested transaction at index: {} of block with number: {}", self.transaction_index, self.block);
        ...
        let tx = RpcTransaction::build(tx.clone(), block_number, block_header.compute_block_hash(), self.transaction_index);
...
impl RpcHandler for GetTransactionByBlockHashAndIndexRequest {
    ...
    fn handle(&self, storage: Store) -> Result<Value, RpcErr> {
        info!("Requested transaction at index: {} of block with hash: {}", self.transaction_index, self.block);
        ...
        let tx = RpcTransaction::build(tx.clone(), block_number, self.block, self.transaction_index);
        serde_json::to_value(tx).map_err(|_| RpcErr::Internal)
    }
}
...
impl RpcHandler for GetTransactionByHashRequest {
    ...
    fn handle(&self, storage: Store) -> Result<Value, RpcErr> {
        ...
        let (block_number, block_hash, index) = match storage.get_transaction_location(self.transaction_hash)? {
            Some(location) => location,
            _ => return Ok(Value::Null),
        };
        let transaction: reec_core::types::Transaction = match storage.get_transaction_by_location(block_hash, index)? {
...
impl RpcHandler for GetTransactionReceiptRequest {
    ...
    fn handle(&self, storage: Store) -> Result<Value, RpcErr> {
        info!("Requested receipt for transaction {}", self.transaction_hash);
        let (block_number, block_hash, index) = match storage.get_transaction_location(self.transaction_hash)? {
            Some(location) => location,
            _ => return Ok(Value::Null)
        };
        ...
        let receipts = block::get_all_block_rpc_receipts(block_number, block.header, block.body, &storage)?;
...
impl RpcHandler for EstimateGasRequest {
    ...
    fn handle(&self, storage: Store) -> Result<Value, RpcErr> {
        ...
        let spec_id = reec_evm::spec_id(&storage, block_header.timestamp)?;
