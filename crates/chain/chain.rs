...
use reec_core::types::{validate_block_header, validate_cancun_header_fields, validate_no_cancun_header_fields, Block, BlockHash, BlockHeader, BlockNumber, EIP4844Transaction, Receipt, Transaction};
use reec_core::H256;
...
pub fn extends_canonical_chain(block: &Block, storage: Store) -> Result<(), ChainError> {
    let latest_block_number = storage.get_latest_block_number()?.ok_or(ChainError::StoreError(StoreError::Custom("Could not find latest valid hash".to_string(),)))?;
...
pub fn store_receipts(storage: &Store, receipts: Vec<Receipt>, block_hash: BlockHash) -> Result<(), ChainError> {
...
pub fn latest_valid_hash(storage: Store) -> Result<H256, ChainError> {
    ...
    Err(ChainError::StoreError(StoreError::Custom("Could not find latest valid hash".to_string(),)))
}
...
/// If a block passes this check, execution will still fail with executre block when a transaction runs out of gas
pub fn validate_block(block: &Block, parent_header: &BlockHeader, state: &EvmState) -> Result<(), ChainError> {
...
fn verify_blob_gas_usage(block: &Block) -> Result<(), ChainError> {
    ...
    if blob_gas_used > MAX_BLOB_GAS_PER_BLOCK {
        return Err(ChainError::InvalidBlock(InvalidBlockError::ExceededMaxBlobGasPerBlock,));
    }
    if blobs_in_block > MAX_BLOB_NUMBER_PER_BLOCK {
        return Err(ChainError::InvalidBlock(InvalidBlockError::ExceededMaxBlobGasPerBlock,));
    }
    if block.header.blob_gas_used.is_some_and(|header_blob_gas_used| header_blob_gas_used != blob_gas_used) {
        return Err(ChainError::InvalidBlock(InvalidBlockError::BlobGasUsedMismatch));
    }
    Ok(())
}
