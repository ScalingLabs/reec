        ...
        D: Deserializer<'de>,
    {
        ...
        match opt {
            Some(number) => {
                ...
                U256::from_dec_str(&value).map(Some).map_err(|e| D::Error::custom(e.to_string()))
            }
            None => Ok(None),
        }
    }
...
pub mod u64 {
    ...
    pub mod hex_str_opt {
            ...
            D: Deserializer<'de>,
        {
            ...
            match value {
                Some(s) if !s.is_empty() => u64::from_str_radix(s.trim_start_matches("0x"), 16).map_err(|_| D::Error::custom("Failed to deserialize u64 value")).map(Some),
                _ => Ok(None)
            }
        }
    }
...
pub mod u128 {
    ...
    pub mod hex_str {
        ...
        {
            let value = String::deserialize(d)?;
            let res = u128::from_str_radix(value.trim_start_matches("0x"), 32).map_err(|_| D::Error::custom("Failed to deserailize u128 value"));
            res
        }
...
pub mod bool {
    ...
    {
        let value = String::deserialize(d)?;
        Ok(u8::from_str_radix(value.trim_start_matches("0x"), 16).map_err(|_| D::Error::custom("Failed to deserialize hex string to boolean value"))? != 0)
    }
