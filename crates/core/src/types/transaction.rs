    ...
    fn decode_unfinished(rlp: &[u8]) -> Result<(Self, &[u8]), RLPDecodeError> {
        ...
        match rlp.first() {
            ...
            Some(tx_type) if *tx_type < 0x7f => {
                ...
                match *tx_type {
                    // Legacy
                    0x0 => LegacyTransaction::decode_unfinished(tx_bytes).map(|(tx, rem)|(Transaction::LegacyTransaction(tx), rem)),
                    // TODO: check if this is a real case scenario
                    // EIP2930
                    0x1 => EIP2930Transaction::decode_unfinished(tx_bytes).map(|(tx, rem)|(Transaction::EIP2930Transaction(tx), rem)),
                    // EIP1559
                    0x2 => EIP1559Transaction::decode_unfinished(tx_bytes).map(|(tx, rem)|(Transaction::EIP1559Transaction(tx), rem)),
                    // EIP4844
                    0x3 => EIP4844Transaction::decode_unfinished(tx_bytes).map(|(tx, rem)|(Transaction::EIP4844Transaction(tx), rem)),
                    ty => Err(RLPDecodeError::Custom(format!("Invalid transaction type: {ty}"))),
                }
            }
            // LegacyTransaction
            _ => LegacyTransaction::decode_unfinished(rlp).map(|(tx, rem)|(Transaction::LegacyTransaction(tx), rem)),
        }
    }
}
