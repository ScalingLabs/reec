use super::{
    ReceiptBlockInfo, BASE_FEE_MAX_CHANGE_DENOMINATOR, ELASTICITY_MULTIPLIER, GAS_LIMIT_ADJUSTMENT_FACTOR, GAS_LIMIT_MINIMUM
};
use crate::{
    rlp::{
        encode::RLPEncode, 
...
pub use serializable::BlockSerializeable;
...
impl RLPDecode for BlockHeader {
    fn decode_unfinished(rlp: &[u8]) -> Result<(Self, &[u8]), crate::rlp::error::RLPDecodeError> {
        ...
        let (parent_beacon_block_root, decoder) = decoder.decode_field("parent_beacon_block_root")?;
...
pub fn compute_receipts_root(receipts: &[Receipt]) -> H256 {
    let receipts_iter: Vec<_> = receipts
        ...
        .map(|(i, receipt)| {
            ...
            // Value: tx_type || RLP(receipt) if tx_type != 0 RLP(receipt) else
            let mut v = Vec::new();
...
pub fn compute_withdrawals_root(withdrawals: &[Withdrawal]) -> H256 {
    ...
    let root = PatriciaMerkleTree<_, _, Keccak256>::compute_hash_from_sorted_iter(&withdrawals_iter);
    H256(root.into())
}
...
mod serializable {
    ...
    impl BlockSerializeable {
        pub fn from_block(
            ...
            BlockSerializeable { hash, header, body}
        }
    }
}
...
mod test {
    ...
    use hex_literal::hex;
    use serializable::BlockSerializeable;
    ...
    #[test]
     fn serialize_block() {
         ...
         let block = BlockSerializeable::from_block(block_header, block_body, true);
