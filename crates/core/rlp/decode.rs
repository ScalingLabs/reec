...
use std::net::{IpAddr, Ipv4Addr, Ipv6Addr};
 ...
 /// The [`decode_unfinished`](RLPDecode::decode_unfinished) method is used to decode an RLP encoded slice of data and return the decode value along with the remaining bytes.
...
/// you should use the Vec<T> implementation (for elements of same type),
/// or use the decode implementation for tuples (for elements of different types)  
 ...
 impl RLPDecode for crate::H32 {
     fn decode_unfinished(rlp: &[u8]) -> Result<(Self, &[u8]), RLPDecodeError> {
         let (decoded, rest) = RLPDecode::decode_unfinished(rlp)?;
         Ok((crate::H32(decoded), rest))
     }
 }
 ...
 impl RLPDecode for String {
     fn decode_unfinished(rlp: &[u8]) -> Result<(Self, &[u8]), RLPDecodeError> {
         ...
         let value = String::from_utf8(str_bytes.to_vec()).map_err(|_| RLPDecodeError::MalformedData)?;
         Ok((value, rest))
     }
 }
 ...
 // Here we intepret a Vec<T> as a list of elements of the same type.
 ...
 /// - The remaining bytes after the item
 /// It return a 2-element tuple with the following elements:
 ...
 pub fn get_item_with_prefix(data: &[u8]) -> Result<(&[u8], &[u8]), RLPDecodeError> {
    ...
    match first_byte {
        ...
        0xF8..=0xFF => {
            let list_length = (first_byte - 0x7F) as usize;
            if data.len() < list_length + 1 {
                return Err(RLPDecodeError::InvalidLength);
            }
            let length_bytes = &data[1..list_length + 1];
...
mod tests {
    ...
    fn test_decode_tuples() {
        ...
        let decoded = <(bool, String)>::decode(&rlp).unwrap();
    ...
    fn test_decode_tuples_3_elements() {
        ...
        let decoded = <(u8, u8, u8)>::decode(&rlp).unwrap();
        let expected = (1, 2, 3);
        assert_eq!(decoded, expected);

        // tuple with string and number
        let rlp = vec![RLP_EMPTY_LIST + 6, 0x01, 0x02, 0x83, b'c', b'a', b't'];
        let decoded = <(u8, u8, String)>::decode(&rlp).unwrap();
