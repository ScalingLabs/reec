use super::{
    BASE_FEE_MAX_CHANGE_DENOMINATOR, BLOB_BASE_FEE_UPDATE_FRACTION, ELASTICITY_MULTIPLIER, GAS_LIMIT_ADJUSTMENT_FACTOR, GAS_LIMIT_MINIMUM, INITIAL_BASE_FEE, MIN_BASE_FEE_PER_BLOB_GAS,
};
use crate::{
    rlp::{
        encode::RLPEncode, 
...
pub struct BlockBody {
    ...
    // TODO: ommers list is always empty, so we remove it
...
fn check_gas_limit(gas_limit: u64, parent_gas_limit: u64) -> bool {
    ...
    gas_limit < parent_gas_limit + max_adjustment_delta && gas_limit > parent_gas_limit - max_adjustment_delta && gas_limit >= GAS_LIMIT_MINIMUM
}

// Calculates the base fee per blob gas for the current block based on its parent excess blob gas
pub fn calculate_base_fee_per_blob_gas(parent_header: &BlockHeader) -> u64 {
    fake_exponential (
        MIN_BASE_FEE_PER_BLOB_GAS,
        parent_header.excess_blob_gas.unwrap_or_default(), BLOB_BASE_FEE_UPDATE_FRACTION,
    )
}
...
fn calculate_base_fee_per_gas(
    ...
    // Check gas limit, if the check passes we can also be rest assured that none of the following divisions will have zero as a divider
    ...
    Some(match parent_gas_used.cmp(&parent_gas_target) {
        ...
        Ordering::Greater => {
            ...
            let base_fee_per_gas_delta = max(target_fee_gas_delta / BASE_FEE_MAX_CHANGE_DENOMINATOR, 1);
...
pub fn validate_block_header(header: &BlockHeader, parent_header: &BlockHeader) -> bool {
    ...
    let expected_base_fee_per_gas = if let Some(base_fee) = calculate_base_fee_per_gas(header.gas_limit, parent_header.gas_limit, parent_header.gas_used, parent_header.base_fee_per_gas.unwrap_or(INITIAL_BASE_FEE),) {
        base_fee
    } else {
        return false;
    };
...
mod test {
    ...
    #[test]
    fn test_compute_withdrawal_root() {
    ...
    fn test_validate_block_header() {
        let parent_block = BlockHeader {
            ...
            receipts_root: H256::from_str("0x56e81f171bcc55a6ff8345e692c0f86e5b48e01b996cadc001622fb5e363b421").unwrap(),
     ...
     fn test_compute_transactions_root() {
        ...
        let transactions: Vec<Transaction> = encoded_transactions.iter().map(|hex| {
            Transaction::decode_canonical(&hex::decode(hex.trim_start_matches("0x")).unwrap()).unwrap()
        })
        .collect();
