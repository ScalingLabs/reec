use super::{
    BASE_FEE_MAX_CHANGE_DENOMINATOR, BLOB_BASE_FEE_UPDATE_FRACTION, ELASTICITY_MULTIPLIER, GAS_LIMIT_ADJUSTMENT_FACTOR, GAS_LIMIT_MINIMUM, MIN_BASE_FEE_PER_BLOB_GAS,
};
use crate::{
    rlp::{
        encode::RLPEncode, 
...
pub fn validate_block_header(header: &BlockHeader, parent_header: &BlockHeader) -> bool {
    ...
    let expected_base_fee_per_gas = if let Some(base_fee) = calculate_base_fee_per_gas(header.gas_limit, parent_header.gas_limit, parent_header.gas_used, parent_header.base_fee_per_gas) {
        base_fee
    } else {
        return false;
    };
