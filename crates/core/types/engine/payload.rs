...
use crate::types::{
    compute_transactions_root, compute_withdrawals_root, Block, BlockBody, BlockHash, BlockHeader, Transaction, Withdrawal, DEFAULT_OMMERS_HASH,
};
...
impl EncodedTransaction {
    /// Based on [EIP-2718]
    /// Transactions can be encoded in the followig formats:
...
impl PayloadStatus {
    //  Convienence methods to create payload status
    ...
    /// Creates a PayloadStatus wth syncing status and no other info
    pub fn syncing() -> Self {
        PayloadStatus { 
...
mod test {
    ...
    #[test]
    fn deserilaize_payload_into_block() {
