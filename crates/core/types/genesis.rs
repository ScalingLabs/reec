...
use super::{
    code_hash, compute_receipts_root, compute_transactions_root, compute_withdrawals_root, AccountInfo, AccountState, Block, BlockBody, BlockHeader, DEFAULT_OMMERS_HASH, INITIAL_BASE_FEE,
};
    ...
    fn get_block_header(&self) -> BlockHeader {
        ...
        if let Some(cancun_time) = self.config.cancun_time {
            if cancun_time <= self.timestamp {
                blob_gas_used = Some(self.blog_gas_used.unwrap_or(0));
        ...
        BlockHeader { 
            ...
            receipts_root: compute_transactions_root(&[]), 
            ...
            withdrawals_root: self.config.is_shanghai_activated(self.timestamp).then_some(compute_transactions_root(&[])),
            blob_gas_used, 
            excess_blob_gas, 
            parent_beacon_block_root: self.config.is_cancun_activated(self.timestamp).then_some(H256::zero()),
        }
    }
...
mod tests {
    ...
    fn deserialize_genesis_file() {
        ...
        let file = File::open("./data/genesis-kurtosis.json").expect("Failed to open genesis file");
    ...
    fn genesis_block() {
        // Deserialize genesis file
        let file = File::open("../../test_data/genesis-kurtosis.json").expect("Failed to open genesis file");
        let reader = BufReader::new(file);
        let genesis: Genesis =
            serde_json::from_reader(reader).expect("Failed to deserialize genesis file");
        ...
        assert_eq!(body.withdrawals.is_some_and(|w| w.is_empty()));
    }
    ...
    fn read_and_compute_kurtosis_hash() {
        ...
        assert_eq!(genesis_block_hash, H256::from_str("0xcb5306dd861d0f2c1f9952fbfbc75a46d0b6ce4f37bea370c3471fe8410bf40b").unwrap())
    }
    ...
    fn read_and_compute_hive_hash() {
        ...
        let genesis_block_hash = H256::from_str("0x30f516e34fc173bb5fc4daddcc7532c4aca10b702c7228f3c806b4df2646fb7e").unwrap();
