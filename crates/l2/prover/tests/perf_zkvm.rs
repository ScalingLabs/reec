...
use reec_core::types::Block;
use std::path::Path;
use tracing::info;

use reec_blockchain::add_block;
use reec_prover_lib::prover::{Prover, Risc0Prover, Sp1Prover};
use reec_storage::{EngineType, Store};
use reec_vm::execution_db::ExecutionDB;
...
async fn setup() -> (ProgramInput, Block) {
    ...
    let genesis =
        reec_l2::utils::test_data_io::read_genesis_file(genesis_file_path.to_str().unwrap());
    store.add_initial_state(genesis.clone()).unwrap();

    let blocks = reec_l2::utils::test_data_io::read_chain_file(chain_file_path.to_str().unwrap());
