...
use bytes::Bytes;
use reec_core::{types::TxKind, Address, H256};
use reec_levm::{
    errors::{TransactionReport, TxResult},
    Account, StorageSlot,
};
use reec_storage::{error::StoreError, AccountUpdate};
use reec_vm::{db::StoreWrapper, EvmState, RevmAddress, RevmU256, SpecId};
use revm::{
    db::State,
...
pub fn ensure_post_state(
    ...
    match test.post.vector_post_value(vector).expect_exception {
        ...
        None => {
            ...
            let revm_account_updates = reec_vm::get_state_transitions(revm_state);
            ...
            match test.post.vector_post_value(vector).expect_exception {
                ...
                None => {
                    ...
                    return Err(EFTestRunnerError::ExecutionFailedUnexpectedly(
                        reec_levm::errors::VMError::AddressAlreadyOccupied,
