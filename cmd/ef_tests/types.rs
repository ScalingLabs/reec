    ...
    fn from(val: Transaction) -> Self {
        match val.transaction_type {
            Some(tx_type) => match tx_type.as_u64() {
                0 => reecTransaction::LegacyTransaction(val.into()),
                1 => reecTransaction::EIP2930Transaction(val.into()),
                2 => reecTransaction::EIP1559Transaction(val.into()),
                3 => reecTransaction::EIP4844Transaction(val.into()),
                _ => unimplemented!(),
            },
...
impl From<Transaction> for EIP4844Transaction {
    fn from(value: Transaction) -> Self {
        EIP4844Transaction { 
            chain_id: val.chain_id.map(|id: U256| id.as_u64()).unwrap_or(1),  // TODO: Consder converting this into Options, 
            nonce: val.nonce.as_u64(), 
            max_priority_fee_per_gas: val.max_priority_fee_per_gas.unwrap_or_default().as_u64(),  // TODO: Consider converting this into Option 
