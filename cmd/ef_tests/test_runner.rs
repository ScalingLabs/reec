...
pub fn run_ef_test(test_key: &str, test: &TestUnit) {
    ...
    store.set_chain_config(chain_config).expect("Failed to set chain config on db");
    ...
    for block_fixture in test.blocks.iter() {
        ...
        match chain_result {
            Err(error) => {
                assert!(expects_exception, "Transaction execution unexpected failed to test: {}, with error {}", test_key, error);
                return;
            }
            Ok(_) => assert!(!expects_exception, "Expect transaction execution to fail in test: {} with error: {}", test_key, block_fixture.expect_exception.clone().unwrap()),
        }
    }
...
fn exception_in_rlp_decoding(block_fixture: &BlockWithRLP) -> bool {
    let expects_rlp_exception = block_fixture.expect_exception.as_ref().map_or(false, |s| s.starts_with("BlockException.RLP_"));
...
pub fn build_store_for_test(test: &TestUnit) -> EvmState {
    let store = Store::new("store.db", EngineType::InMemory).expect("Failed to build DB for testing");
...
/// Panics if any comparison fails
fn check_poststate_against_db(test_key: &str, test: &TestUnits, db: &Store) {
    for (addr, account) in &test.post_state {
        ...
        for (key, value) in expected_account.storage {
            let db_storage_value = db
                ...
                .expect("Failed to read from DB")
                .unwrap_or_else(|| {panic!("Storage missing for address {addr} key {key} in DB test:{test_key}")});
            assert_eq!(
