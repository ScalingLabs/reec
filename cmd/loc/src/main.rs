...
fn main() {
    ...
    let reec = PathBuf::from(CARGO_MANIFEST_DIR).join("../../");
    let levm = PathBuf::from(CARGO_MANIFEST_DIR).join("../../crates/vm");
    let reec_l2 = PathBuf::from(CARGO_MANIFEST_DIR).join("../../crates/l2");

    let config = Config::default();

    let mut languages = Languages::new();
    languages.get_statistics(&[reec.clone()], &["tests"], &config);
    let reec_loc = &languages.get(&LanguageType::Rust).unwrap();
    ...
    let mut languages = Languages::new();
    languages.get_statistics(&[reec_l2], &["tests"], &config);
    let reec_l2_loc = &languages.get(&LanguageType::Rust).unwrap();
    ...
    let new_report = LinesOfCodeReport {
        reec: reec_loc.code,
        reec_l1: reec_loc.code - reec_l2_loc.code - levm_loc.code,
        reec_l2: reec_l2_loc.code,
        levm: levm_loc.code,
    };
