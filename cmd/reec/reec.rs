...
use reec_blockchain::{add_block, fork_choice::apply_fork_choice};
use reec_core::{
    types::{Block, Genesis},
    H256,
};
...
const DEFAULT_DATADIR: &str = "reec";
#[tokio::main]
async fn main() {
    ...
    cfg_if::cfg_if! {
        if #[cfg(feature = "l2")] {
            let l2_proposer = reec_l2::start_proposer(store).into_future();
            ...
            let block_producer_engine = reec_dev::block_producer::start_block_producer(url, authrpc_jwtsecret.into(), head_block_hash, max_tries, 1000, reec_core::Address::default());
            tracker.spawn(block_producer_engine);
        } else {
            let networking = reec_net::start_network(
                udp_socket_addr,
                tcp_socket_addr,
                bootnodes,
                signer,
