...
pub struct WalletConfig {
    pub address: Address,
    #[serde(
        serialize_with = "reec_l2::utils::secret_key_serializer",
        deserialize_with = "reec_l2::utils::secret_key_deserializer"
    )]
...
pub async fn try_load_selected_config() -> eyre::Result<Option<ReecL2Config>> {
...
pub async fn load_selected_config() -> eyre::Result<ReecL2Config> {
    let config_path = selected_config_path()?;
    if !config_path.exists() {
