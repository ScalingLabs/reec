use crate::{config::ReecL2Config, utils::config::confirm};
use clap::Subcommand;
...
impl Command {
    pub async fn run(self, cfg: ReecL2Config) -> eyre::Result<()> {
        ...
        let reec_dev_path = root.join("crates/blockchain/dev");
        ...
        match self {
            Command::Init {
                ...
                if l1_rpc_url.contains("localhost") {
                    start_l1(&l2_crate_path, &reec_dev_path).await?;
...
fn shutdown_l1(reec_dev_path: &Path) -> eyre::Result<()> {
    let local_l1_docker_compose_path = reec_dev_path.join("docker-compose-dev.yaml");
    let cmd = std::process::Command::new("docker")
        .arg("compose")
        .arg("-f")
        .arg(local_l1_docker_compose_path)
        .arg("down")
        .current_dir(reec_dev_path)
        .spawn()?
        .wait()?;
    if !cmd.success() {
...
async fn start_l1(l2_crate_path: &Path, reec_dev_path: &Path) -> eyre::Result<()> {
    create_volumes(l2_crate_path)?;
    docker_compose_l2_up(reec_dev_path)?;
    Ok(())
}
...
fn docker_compose_l2_up(reec_dev_path: &Path) -> eyre::Result<()> {
    let local_l1_docker_compose_path = reec_dev_path.join("docker-compose-dev.yaml");
    let cmd = std::process::Command::new("docker")
        .arg("compose")
        .arg("-f")
