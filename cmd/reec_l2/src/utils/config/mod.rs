use crate::{
    ...
    config::{ContractsConfig, ReecL2Config, NetworkConfig, WalletConfig},
    utils::messages::{
...
pub fn prompt_config() -> eyre::Result<ReecL2Config> {
    let prompted_config = ReecL2Config {
...
pub fn edit_config_by_name_interactively(config_path: &PathBuf) -> eyre::Result<ReecL2Config> {
    let existing_config: ReecL2Config = toml::from_str(&std::fs::read_to_string(config_path)?)?;
...
pub fn edit_existing_config_interactively(
    existing_config: ReecL2Config,
) -> eyre::Result<ReecL2Config> {
    let config = ReecL2Config {
        network: NetworkConfig {
